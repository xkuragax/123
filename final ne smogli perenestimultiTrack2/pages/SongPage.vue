  ...
  <div class="hidden md:block min-h-screen bg-darker overflow-hidden">
    ...
    <div v-else class="container mx-auto px-4 py-3 max-w-4xl">
      ...
      <div class="mb-6 flex items-center gap-3">
        ...
        <a 
          v-if="album"
          :href="albumPageRoute({ id: album.id }).url()" 
          class="inline-flex items-center gap-2 px-4 py-2 bg-dark hover:bg-neutral-800 rounded-xl transition-colors text-text-secondary hover:text-text-primary"
        >
...
import { albumPageRoute } from '../album'
import { lyricsPageRoute } from '../lyrics'
...
const loadSongData = async () => {
  try {
    ...
    console.log('Loading song data for songId:', props.songId)
    ...
    tracks.value = data.tracks.map(t => {
      console.log('Processing track:', t)
      return {
        id: t.id,
        name: t.name,
        fileHash: t.fileHash,
    ...
    console.log('Final tracks for player:', tracks.value)
    loading.value = false
  } catch (err) {
